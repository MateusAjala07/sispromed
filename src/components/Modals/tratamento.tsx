...
const defaultValoresFormulario: FormFieldsTratamento = {
  nome: "",
};

export default function ModalTratamento({
  isOpen,
  setIsOpen,
  acao,
  id,
  reload,
}: ModalTratamentoProps) {
  ...
  async function onSubmit(data: z.infer<typeof formSchema>) {
    try {
      ...
      if (error instanceof AxiosError) {
        toast.error(
          error.response?.data?.message ?? "Erro ao salvar tratamento"
        );
      }
    }
  }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await consultarTratamento(id);
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent>
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <DialogHeader>
              <DialogTitle>
                {acao === "criar" ? "Novo Tratamento" : "Editar Tratamento"}
              </DialogTitle>
              <DialogDescription>
                Insira informações do tratamento
              </DialogDescription>
            </DialogHeader>
