...
const defaultValoresFormulario: FormFieldsConvenio = {
  nome: "",
};

export default function ModalConvenio({
  isOpen,
  setIsOpen,
  acao,
  id,
  ...
  async function onSubmit(data: z.infer<typeof formSchema>) {
    try {
      ...
      if (error instanceof AxiosError) {
        toast.error(error.response?.data?.message ?? "Erro ao salvar convênio");
      }
    }
  }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await consultarConvenio(id);
        ...
        <DialogContent>
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <DialogHeader>
              ...
              <DialogDescription>
                Insira informações do convênio
              </DialogDescription>
            </DialogHeader>
